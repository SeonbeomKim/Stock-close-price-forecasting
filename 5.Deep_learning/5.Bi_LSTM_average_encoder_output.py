...
class model:
	def __init__(self, sess):
		...
		#concat mean of encoder output with decoder output
		self.concat_vector = self.concat_mean_encoder_output_with_decoder_output(self.en_output, self.de_output) # batch, cell_num*2
	

		#prediction
		self.pred = self.prediction(self.concat_vector) # batch, target
	...
	def concat_mean_encoder_output_with_decoder_output(self, en_output, de_output):
		...
		concat_vector = tf.nn.tanh(tf.matmul(concat, W_concat) + W_bias) # batch, cell_num*2
