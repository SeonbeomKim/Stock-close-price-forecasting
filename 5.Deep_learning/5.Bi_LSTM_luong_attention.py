...
tensorboard_path = './tensorboard_luong_attention/'
saver_path = './saver_luong_attention/'
...
class model:
	def __init__(self, sess):
		...
		#encoder
		self.en_output, self.en_state = self.bidirectional_encoder(self.X_reshape) #bidirectional, en_output = batch, gram(20), cell_num*2	


		#decoder
		self.de_output = self.decoder(tf.shape(self.en_output)[0], self.en_state) # batch, 1, cell_num*2


		#Luong attention
		self.attention_vector = self.Luong_Attention(self.en_output, self.de_output) # batch, cell_num*2
	

		#prediction
		self.pred = self.prediction(self.attention_vector) # batch, target
	...
	def bidirectional_encoder(self, X_reshape):
		...
		en_output = tf.concat((en_fw_val, en_bw_val), 2) 
		en_state_c = tf.concat((en_fw_state.c, en_bw_state.c), 1)
		en_state_h = tf.concat((en_fw_state.h, en_bw_state.h), 1)
		en_state = tf.contrib.rnn.LSTMStateTuple(c = en_state_c, h = en_state_h)
	...
	def Luong_Attention(self, en_output, de_output):
		#calc a
		a_score = en_output * de_output # => batch, gram(20), cell_num*2
		a_dot = tf.reduce_sum(a_score, axis=2, keep_dims=True) # batch, gram(20), 1
		a_align = tf.nn.softmax(a_dot, dim=1) # batch, gram(20), 1	
		
		#calc c  (weighted average)
		context_vector = en_output * a_align #batch, gram(20), cell_num*2
		context_vector = tf.reduce_mean(context_vector, axis=1, keep_dims=True) # batch, 1, cell_num*2
		...
		W_bias = tf.Variable(tf.constant(1.0, shape = [cell_num*2])) #in paper there is no bias
		
		attention_vector = tf.nn.tanh(tf.matmul(concat, W_concat) + W_bias) # batch, cell_num*2

		return attention_vector



	def prediction(self, attention_vector):
		W = tf.get_variable('w', shape = [cell_num*2, target_size], 
				initializer=tf.contrib.layers.xavier_initializer())
		bias = tf.Variable(tf.constant(1.0, shape = [target_size]))	
		pred = tf.matmul(attention_vector, W) + bias
